...
class AttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        if value_size is None:
            value_size = key_size  # Usually key and values are the same
    ...
    def __repr__(self):
        template = '({} {} value_size={} mode={} \
            normalize={} dropout={} batch_first={} \
            weight_norm={} bias={} query_transform={} output_transform={} \
            output_nonlinearity={} output_size={})'
        options = template.format(self.query_size, self.key_size, self.value_size, self.mode, \
            self.normalize, self.dropout_ratio, self.batch_first, \
            self.weight_norm, self.bias, self.query_transform, self.output_transform, \
            self.output_nonlinearity, self.output_size)
        return self.__class__.__name__ + options
...
class SDPAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        if self.causal:
            ...
            mask = Variable(causal_mask.unsqueeze(0).expand(b, t_q, t_k), requires_grad=False)
