...
class SpatialPyramidPoolingLayer(torch.nn.Module):
    ...
    def forward(self, previous_conv, previous_conv_size):
        ...
        for i in range(len(self.out_pool_size)):
            h_wid = int(
                math.ceil(previous_conv_size[0] / self.out_pool_size[i]))
            w_wid = int(
                math.ceil(previous_conv_size[1] / self.out_pool_size[i]))
            h_pad = (
                h_wid * self.out_pool_size[i] - previous_conv_size[0] + 1) / 2
            w_pad = (
                w_wid * self.out_pool_size[i] - previous_conv_size[1] + 1) / 2
